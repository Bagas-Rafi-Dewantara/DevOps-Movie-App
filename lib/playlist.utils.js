const validateCreate = ({ userId, name } = {}) => {
  ...
  if (typeof name !== "string" || name.trim().length === 0) return { valid: false, status: 400 };
  return { valid: true, status: 201 };
};

const isValidPlaylistName = (name) => typeof name === "string" && name.trim().length > 0;

const isMovieInPlaylist = (movies, movieId) => movies.some((m) => m.movieId === movieId);
...
const removeMovieFromPlaylist = (movies, movieId) => movies.filter((m) => m.movieId !== movieId);
