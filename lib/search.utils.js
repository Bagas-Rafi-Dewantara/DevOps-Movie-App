const filterByGenre = (movies, genreId) => movies.filter((m) => m.genre_ids.includes(genreId));

const filterByYear = (movies, year) =>
  movies.filter((m) => new Date(m.release_date).getFullYear() === year);

const sortByRatingDesc = (movies) => [...movies].sort((a, b) => b.vote_average - a.vote_average);

const sortByRatingAsc = (movies) => [...movies].sort((a, b) => a.vote_average - b.vote_average);
